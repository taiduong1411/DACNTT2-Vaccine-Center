...
function VaccineManager() {
                ...
                return (
                    <div>
                        ...
                        <span style={{ marginLeft: '10px' }}>
                            <Link to={`/doctor/vaccine-manager/vaccine-detail/${record.slug}/${record.pro_code}`}><strong>[{record.pro_code}]</strong> {record.pro_name}</Link>
                        </span>
                    </div>
                );
            }
    ...
    const getDataVaccine = async () => {
        await axiosCli().get('doctor/all-vaccines').then(res => {
    ...
    const handleCenterData = async () => {
        await axiosCli().get('doctor/all-centers').then(res => {
    ...
    const onAddSubmit = async (data) => {
        ...
        // console.log(allData);
        await axiosCli().post('doctor/add-vaccine', allData).then(res => {
            if (res.status == 200) {
                handleCancel();
                getDataVaccine();
    ...
    const deleteProduct = async (_id) => {
        await axiosCli().del(`doctor/del-vaccine/${_id}`).then(res => {
            if (res.status == 200) {
                success(res.data.msg);
                getDataVaccine();
            } else {
    ...
    const showUpdate = async (event) => {
        ...
        await axiosCli().get(`doctor/get-vaccine/${_id}`).then(res => {
            if (res.status == 200) {
    ...
    const onUpdateSubmit = async (data) => {
        ...
        await axiosCli().post(`doctor/update-vaccine/${dataPro._id}`, allDataUpdate).then(res => {
            if (res.status == 200) {
                handleCancel();
                getDataVaccine();
                success(res.data.msg);
    ...
    const onSearchSubmit = async (data) => {
        await axiosCli().get(`doctor/search-vaccine/${data.key}`).then(res => {
