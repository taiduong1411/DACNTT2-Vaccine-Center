...
const BlogManagerDoctor = () => {
    ...
    const getDataBlog = async () => {
        await axiosCli().get('doctor/all-blogs').then(res => {
    ...
    const onAddSubmit = async (data) => {
        ...
        await axiosCli().post('doctor/add-blog', allData).then(res => {
            if (res.status == 200) {
                messageApi.open({
                    type: 'success',
    ...
    const deleteBlog = async (_id) => {
        await axiosCli().del(`doctor/del-blog/${_id}`).then(res => {
            if (res.status == 200) {
                messageApi.open({
                    type: 'success',
                    content: res.data.msg
    ...
    const onSearchSubmit = async (data) => {
        await axiosCli().get(`doctor/search-blog/${data.key}`).then(res => {
            if (res.data.length < 1) {
                setDataBlog([]);
                messageApi.open({
                    type: 'warning',
                    content: 'Không tìm thấy blog'
                })
            } else {
                setDataBlog(res.data)
            }
    ...
    const showUpdateOpen = async (event) => {
        ...
        setIdUpdate(id);
        await axiosCli().get(`doctor/blog-by-id/${id}`).then(async res => {
    ...
    const onUpdateSubmit = async (data) => {
        ...
        await axiosCli().post(`doctor/update-blog/${idUpdate}`, allDataUpdate).then(res => {
            if (res.status == 200) {
                messageApi.open({
                    type: 'success',
                    content: res.data.msg
...
export default BlogManagerDoctor;