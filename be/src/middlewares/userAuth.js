...
module.exports = userRole = async (req, res, next) => {
    ...
    await jwt.verify(token, process.env.SESSION_SECRET, function (err) {
        if (err) {
            ...
            if (token_decode.level == '3' || token_decode.level == '2') {
                return res.status(404).json({ msg: 'Bạn Không Thể Sử Dụng Chức Năng Của Người Dùng. Vui Lòng Sử Dụng Tài Khoản Khác' })
            } else {
                next();
            }
        }
